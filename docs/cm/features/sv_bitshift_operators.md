...
Cm SV バックエンドにおけるビットシフト演算子 (`<<`, `>>`) のSystemVerilog コード生成対応。
...
`uart_hello.cm` および `uart_button.cm` では、ビットシフトの代わりに除算 (`/`) とビットマスク (`&`) を組み合わせて使用している:
...
**HIGH** — TMDS エンコーダの効率的な実装に不可欠。回避策 (除算 + マスク) は動作するが、合成リソースが増大し、コードの可読性が著しく低下する。
