...
## 背景と目的
前回のリファクタリングにより、`animation_ctrl.cm` 内の制御文字略記アスキーパック定義を `const` 定数として可読化しました。本設計では、定数定義が `animation_ctrl.cm` 内に多く存在している状態を解消し、文字管理をより容易にするため、これらの定数を別のモジュールファイル `ctrl_abbrev.cm` に分割します。これにより、文字の追加・変更が1箇所で管理可能となり、`animation_ctrl.cm` は本来の制御・描画ロジックのみに集中することができます。
